...
| Do | Don't |
|----|-------|
| Use one Primary button for the single most important action in a view. | Use two Primary buttons competing for attention in the same view. |
| Write short, verb-led labels: `<Button>Save changes</Button>` | Write long, passive labels: `<Button>Changes will be saved</Button>` |
| Keep Outline's border visible in every state, including Disabled. | Let Outline's border disappear because the background is already white. |
