...
> **Experimental branch note:** built the same way as TextField — on
...
| `value` / `defaultValue` | `Key \| null`                                | —       | Controlled / uncontrolled selected option id. |
| `onChange`     | `(value: Key \| null) => void`                         | —       | Fires when the selection changes. |
